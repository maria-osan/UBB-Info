...
void Console::print(const vector_dinamic_t<Disciplina>& d_lst)
{
	cout << "\nDiscipline: \n";
	/*for (&d : d_lst)
	{
	...
	for (int i = 0; i < d_lst.size(); i++)
	{
		cout << " - " << d_lst.get_elem(i).get_id() << ": " << d_lst.get_elem(i).get_name() << ", " << d_lst.get_elem(i).get_nr_hours() << ", " << d_lst.get_elem(i).get_type() << ", " << d_lst.get_elem(i).get_teacher() << "\n";
	}
	cout << "\n";
}
		...
		try {
			...
			else if (cmd == '6')
			{
				...
				if (opt == '1')
				{
					...
					getline(cin, nr_hours);
					print(srv.filter_equal_hours<Disciplina>(nr_hours));
				}
				else if (opt == '2')
				{
					string teacher;
					...
					getline(cin, teacher);
					print(srv.filter_same_teacher<Disciplina>(teacher));
				}
				else
